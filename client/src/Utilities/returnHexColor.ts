...
export function returnHexColor(
  ...
  const matchingPigments = fetchedPigments.filter((pigment) =>
  ...
  if (hexValues.length === 1 && hexValues[0].isBase) {
    return "#fff";
  }
  ...
  const hexValue = cmykToHex(totalC, totalM, totalY, totalK);
...
function adjustPercentages(components: FormulaComponent[]): FormulaComponent[] {
  ...
  const totalRoundedPercentage = roundedComponents.reduce(
